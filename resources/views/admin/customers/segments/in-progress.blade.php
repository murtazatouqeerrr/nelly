...
<body>
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">In Progress</h1>
            <p class="text-gray-600 mt-2">Students actively working on courses</p>
        </div>
                    ...
                    <td class="px-6 py-4">
                        <div class="flex items-center">
                            <div class="w-24 bg-gray-200 rounded-full h-2 mr-2">
                                <div class="bg-purple-600 h-2 rounded-full" style="width: {{ $enrollment->progress_percentage }}%"></div>
                            </div>
                            <span>{{ $enrollment->progress_percentage }}%</span>
                        </div>
                    </td>
                    <td class="px-6 py-4">{{ $enrollment->last_activity_at ? $enrollment->last_activity_at->diffForHumans() : 'N/A' }}</td>
                    <td class="px-6 py-4"><a href="{{ route('admin.enrollments.edit', $enrollment->id) }}" class="text-blue-600 hover:text-blue-900">View</a></td>
                </tr>
                @empty
                <tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">No in-progress enrollments found</td></tr>
                @endforelse
            </tbody>
        </table>
