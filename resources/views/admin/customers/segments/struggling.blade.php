...
<head>
    ...
    <title>Struggling Students</title>
...
<body>
    ...
    <div class="mb-6 flex justify-between items-center">
        <div>
            <h1 class="text-3xl font-bold text-gray-900">Struggling Students</h1>
            <p class="text-gray-600 mt-2">Students with {{ $failedAttempts }}+ failed quiz attempts</p>
        </div>
                ...
                <tr>
                    ...
                    <td class="px-6 py-4">
                        <span class="px-2 py-1 text-xs rounded bg-red-100 text-red-800">
                            {{ $enrollment->quizAttempts->where('passed', false)->count() }} failed
                        </span>
                    </td>
                    <td class="px-6 py-4">{{ $enrollment->quizAttempts->last()?->created_at?->diffForHumans() ?? 'N/A' }}</td>
                    <td class="px-6 py-4"><a href="{{ route('admin.enrollments.edit', $enrollment->id) }}" class="text-blue-600 hover:text-blue-900">View</a></td>
                </tr>
                @empty
                <tr><td colspan="5" class="px-6 py-4 text-center text-gray-500">No struggling students found</td></tr>
                @endforelse
            </tbody>
        </table>
